...
public class Security {
	...
	public @ResponseBody ResponseEntity<Serializable> login(
			@RequestBody LoginForm loginForm) throws CarrosUserNotFound,
			CarrosUsuarioNaoTemPapel {
	...
	public @ResponseBody ResponseEntity<String> sair(
	...
	private void iniciarSessao(Serializable result)
			throws CarrosUsuarioNaoTemPapel {
		if (result instanceof Concessionaria) {
			Concessionaria concessionaria = (Concessionaria) result;
			sessaoAtributoService.iniciarSessao(new UsuarioSessao(
					concessionaria.getUsuario().getIdUsuario(),
					TipoUsuarioSessao.CONCESSIONARIA));
		} else if (result instanceof UsuarioConcessionaria) {
			UsuarioConcessionaria usuarioConcessionaria = (UsuarioConcessionaria) result;
			sessaoAtributoService.iniciarSessao(new UsuarioSessao(
					usuarioConcessionaria.getUsuario().getIdUsuario(),
					TipoUsuarioSessao.USUARIO_CONCESSIONARIA));
		} else if (result instanceof Lojista) {
			Lojista lojista = (Lojista) result;
			sessaoAtributoService.iniciarSessao(new UsuarioSessao(lojista
					.getUsuario().getIdUsuario(), TipoUsuarioSessao.LOJISTA));
		} else {
			throw new CarrosUsuarioNaoTemPapel(
					"este usuario não se enquadra no sistema");
		}
	...
	@Autowired
	public void setSessaoAtributoService(
			SessaoAtributoService sessaoAtributoService) {
